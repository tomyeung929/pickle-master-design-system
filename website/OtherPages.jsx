// Pickle Master — Social, Private, Contact, Login, Account Pages
    ...
    <div style={{ background:'#F5F0E8' }}>
      <section style={{ background:'#0F3D24', padding:'80px 24px 60px', textAlign:'center', position:'relative', overflow:'hidden' }}>
        <div style={{ position:'absolute', inset:0, opacity:0.04 }}>
          <svg width="100%" height="100%" viewBox="0 0 600 300" preserveAspectRatio="xMidYMid slice">
            {[0,1,2,3,4].map(i=><circle key={i} cx={120+i*90} cy={150} r={40+i*10} fill="none" stroke="#C9A84C" strokeWidth="1.5" opacity={0.6-i*0.1}/>)}
          </svg>
        </div>
        <div style={{ position:'relative', maxWidth:700, margin:'0 auto' }}>
          ...
          <h1 style={{ fontFamily:"'Playfair Display SC',serif", fontSize:'clamp(32px,5vw,60px)', fontWeight:700, color:'#F5F0E8', lineHeight:1.1, marginBottom:20 }}>{t.social_hero}</h1>
          <p style={{ fontFamily:"'DM Sans',sans-serif", fontSize:16, color:'#9E8E78', maxWidth:520, margin:'0 auto', lineHeight:1.8 }}>{t.social_desc}</p>
        </div>
      ...
      <section style={{ padding:'80px 24px', maxWidth:900, margin:'0 auto' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))', gap:32 }}>
          {[
            ...
            <div key={num} style={{ background:'#fff', borderRadius:12, padding:'28px', boxShadow:'0 4px 16px rgba(15,61,36,0.08)', borderTop:'3px solid #C9A84C' }}>
              <div style={{ fontFamily:"'Playfair Display SC',serif", fontSize:36, fontWeight:700, color:'rgba(201,168,76,0.15)', lineHeight:1, marginBottom:8 }}>{num}</div>
      ...
      <section style={{ background:'#1C4F3A', padding:'64px 24px' }}>
        <div style={{ maxWidth:600, margin:'0 auto', textAlign:'center' }}>
          <div style={{ fontFamily:"'Oswald',sans-serif", fontSize:11, letterSpacing:'0.2em', textTransform:'uppercase', color:'#C9A84C', marginBottom:16 }}>{t.social_price_title}</div>
          <div style={{ display:'flex', gap:24, justifyContent:'center', marginBottom:40, flexWrap:'wrap' }}>
            {[
              ...
              <div key={label} style={{ background:'rgba(255,255,255,0.07)', border:'1px solid rgba(201,168,76,0.2)', borderRadius:12, padding:'24px 32px', minWidth:180 }}>
                ...
                <div style={{ fontFamily:"'Playfair Display SC',serif", fontSize:32, fontWeight:700, color:'#F5F0E8', marginBottom:4 }}>{p}</div>
                <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:11, color:'#9E8E78' }}>{sub}</div>
              </div>
            ))}
          </div>
          <button onClick={book} style={{ background:'#C9A84C', color:'#0F3D24', border:'none', borderRadius:999, padding:'16px 48px', fontFamily:"'Oswald',sans-serif", fontSize:14, fontWeight:700, letterSpacing:'0.14em', textTransform:'uppercase', cursor:'pointer', boxShadow:'0 4px 20px rgba(201,168,76,0.4)' }}>
    ...
    <div style={{ background:'#F5F0E8' }}>
      <section style={{ background:'#0F3D24', padding:'80px 24px 60px', position:'relative', overflow:'hidden' }}>
        <div style={{ maxWidth:700 }}>
          ...
          <h1 style={{ fontFamily:"'Playfair Display SC',serif", fontSize:'clamp(28px,4vw,52px)', fontWeight:700, color:'#F5F0E8', lineHeight:1.15, marginBottom:20 }}>{t.private_hero}</h1>
          <p style={{ fontFamily:"'DM Sans',sans-serif", fontSize:16, color:'#9E8E78', maxWidth:540, lineHeight:1.8, marginBottom:32 }}>{t.private_desc}</p>
      ...
      {/* Partners */}
      <section style={{ background:'#fff', padding:'40px 24px', borderBottom:'1px solid #F5F0E8' }}>
        <div style={{ maxWidth:1000, margin:'0 auto', textAlign:'center' }}>
          <div style={{ fontFamily:"'Oswald',sans-serif", fontSize:10, letterSpacing:'0.2em', textTransform:'uppercase', color:'#9E8E78', marginBottom:24 }}>{t.private_trusted}</div>
          <div style={{ display:'flex', gap:32, justifyContent:'center', flexWrap:'wrap', alignItems:'center' }}>
            {partners.map(p => (
              <div key={p} style={{ fontFamily:"'Playfair Display SC',serif", fontSize:16, fontWeight:700, color:'#D6CBBA', letterSpacing:'0.08em' }}>{p}</div>
            ))}
          </div>
        </div>
      </section>
      ...
      <section style={{ padding:'80px 24px', maxWidth:1100, margin:'0 auto' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))', gap:24 }}>
          {offerings.map(({ title, body }) => (
            <div key={title} style={{ background:'#fff', borderRadius:12, padding:'32px', boxShadow:'0 4px 16px rgba(15,61,36,0.08)' }}>
              <div style={{ width:40, height:40, background:'rgba(201,168,76,0.1)', borderRadius:8, marginBottom:16, display:'flex', alignItems:'center', justifyContent:'center' }}>
...
function ContactPage({ lang }) {
  ...
  return (
    <div style={{ background:'#F5F0E8' }}>
      <section style={{ background:'#0F3D24', padding:'64px 24px 48px' }}>
      ...
      <section style={{ padding:'64px 24px', maxWidth:1000, margin:'0 auto', display:'grid', gridTemplateColumns:'1fr 360px', gap:48 }} className="contact-main-grid">
        ...
        <div>
          {sent ? (
            <div style={{ background:'#fff', borderRadius:12, padding:'40px', textAlign:'center', boxShadow:'0 4px 16px rgba(15,61,36,0.08)' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:24 }}>
          {[
            ...
            <div key={label} style={{ background:'#fff', borderRadius:12, padding:'24px', boxShadow:'0 2px 10px rgba(15,61,36,0.07)' }}>
          ...
          <div style={{ background:'#0F3D24', borderRadius:12, padding:'24px' }}>
            ...
            <div style={{ display:'flex', gap:12 }}>
              {['Instagram','Facebook'].map(s => (
                <a key={s} href="#" style={{ background:'rgba(201,168,76,0.1)', border:'1px solid rgba(201,168,76,0.3)', borderRadius:8, padding:'10px 16px', fontFamily:"'Oswald',sans-serif", fontSize:11, letterSpacing:'0.08em', textTransform:'uppercase', color:'#C9A84C', textDecoration:'none' }}>{s}</a>
              ))}
            </div>
          </div>
        </div>
...
function LoginPage({ lang, setUser, setPage }) {
  ...
  return (
    <div style={{ background:'#F5F0E8', minHeight:'80vh', display:'flex', alignItems:'center', justifyContent:'center', padding:'48px 24px' }}>
      <div style={{ width:'100%', maxWidth:440 }}>
        ...
        <div style={{ background:'#fff', borderRadius:16, boxShadow:'0 8px 32px rgba(15,61,36,0.12)', overflow:'hidden' }}>
          <div style={{ display:'flex', borderBottom:'1px solid #F5F0E8' }}>
            {[['login',t.login_title],['register',t.register_title]].map(([m, label]) => (
              <button key={m} onClick={() => setMode(m)} style={{ flex:1, padding:'16px', border:'none', background:'none', cursor:'pointer', fontFamily:"'Oswald',sans-serif", fontSize:12, letterSpacing:'0.1em', textTransform:'uppercase', color: mode===m ? '#0F3D24' : '#9E8E78', borderBottom: mode===m ? '2px solid #C9A84C' : '2px solid transparent', marginBottom:-1 }}>{label}</button>
            ))}
          </div>
          <form onSubmit={submit} style={{ padding:'28px 28px', display:'flex', flexDirection:'column', gap:16 }}>
            {mode==='register' && (
              <div>
            ...
            <div>
              <div style={{ fontFamily:"'Oswald',sans-serif", fontSize:10, letterSpacing:'0.12em', textTransform:'uppercase', color:'#9E8E78', marginBottom:6 }}>{t.login_email}</div>
              <input required type="email" placeholder="you@picklemaster.hk" value={form.email} onChange={e=>setForm(f=>({...f,email:e.target.value}))} style={{ width:'100%', padding:'12px 14px', border:'1.5px solid #D6CBBA', borderRadius:6, fontFamily:"'DM Sans',sans-serif", fontSize:14, color:'#1A1208', outline:'none', boxSizing:'border-box' }} />
            </div>
            <div>
              <div style={{ fontFamily:"'Oswald',sans-serif", fontSize:10, letterSpacing:'0.12em', textTransform:'uppercase', color:'#9E8E78', marginBottom:6 }}>{t.login_password}</div>
              <input required type="password" placeholder="••••••••" value={form.password} onChange={e=>setForm(f=>({...f,password:e.target.value}))} style={{ width:'100%', padding:'12px 14px', border:'1.5px solid #D6CBBA', borderRadius:6, fontFamily:"'DM Sans',sans-serif", fontSize:14, color:'#1A1208', outline:'none', boxSizing:'border-box' }} />
            </div>
            ...
            <button type="submit" style={{ background:'#C9A84C', color:'#0F3D24', border:'none', borderRadius:999, padding:'15px 0', fontFamily:"'Oswald',sans-serif", fontSize:13, fontWeight:700, letterSpacing:'0.12em', textTransform:'uppercase', cursor:'pointer', marginTop:8, boxShadow:'0 2px 12px rgba(201,168,76,0.35)' }}>
      ...
      <div style={{ maxWidth:900, margin:'0 auto', padding:'40px 24px', display:'grid', gridTemplateColumns:'1fr 300px', gap:32 }} className="account-main-grid">
        ...
        <div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            {bookings.map((b,i) => (
              <div key={i} style={{ background:'#fff', borderRadius:12, padding:'18px 20px', boxShadow:'0 2px 10px rgba(15,61,36,0.08)', display:'flex', alignItems:'center', gap:16, borderLeft:'3px solid #C9A84C' }}>
                <div style={{ flex:1 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          <div style={{ background:'#fff', borderRadius:12, padding:'20px', boxShadow:'0 2px 10px rgba(15,61,36,0.07)' }}>
