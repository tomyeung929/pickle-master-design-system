// Pickle Master — Shared Components: Nav, Footer, Cart, Modals
...
function Nav({ page, setPage, lang, setLang, user, setUser, cart, setCartOpen, mobileOpen, setMobileOpen }) {
  ...
  return (
    <>
      {/* Promo banner */}
      <div style={{ background: '#C9A84C', padding: '8px 24px', textAlign: 'center', fontFamily: "'Oswald',sans-serif", fontSize: 11, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#0F3D24', fontWeight: 600, cursor:'pointer' }} onClick={() => setPage('member')}>
        {t.promo_banner}
      </div>
      {/* Main nav */}
      <nav style={{ background: '#0F3D24', borderBottom: '1px solid rgba(201,168,76,0.2)', position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', flexShrink: 0 }} onClick={() => setPage('home')}>
            ...
            <div>
              <div style={{ fontFamily: "'Playfair Display SC',serif", fontSize: 15, fontWeight: 700, color: '#F5F0E8', lineHeight: 1 }}>Pickle Master</div>
          ...
          <div style={{ display: 'flex', gap: 4, alignItems: 'center' }} className="desktop-nav">
            {navItems.map(({ id, label }) => (
              <button key={id} onClick={() => setPage(id)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'Oswald',sans-serif", fontSize: 12, letterSpacing: '0.12em', textTransform: 'uppercase', color: page === id ? '#C9A84C' : '#D6CBBA', padding: '8px 12px', borderBottom: page === id ? '2px solid #C9A84C' : '2px solid transparent', transition: 'all 0.15s' }}>
                {label}
              </button>
            ))}
            {/* MORE dropdown */}
            <div ref={moreRef} style={{ position: 'relative' }}>
              <button onClick={() => setMoreOpen(o => !o)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'Oswald',sans-serif", fontSize: 12, letterSpacing: '0.12em', textTransform: 'uppercase', color: extraPages.includes(page) ? '#C9A84C' : '#D6CBBA', padding: '8px 12px', borderBottom: extraPages.includes(page) ? '2px solid #C9A84C' : '2px solid transparent', display: 'flex', alignItems: 'center', gap: 4, transition: 'all 0.15s' }}>
              ...
              {moreOpen && (
                <div style={{ position: 'absolute', top: '100%', right: 0, background: '#0A1A0F', border: '1px solid rgba(201,168,76,0.2)', borderRadius: 8, minWidth: 160, boxShadow: '0 8px 24px rgba(0,0,0,0.3)', zIndex: 200, overflow: 'hidden', marginTop: 4 }}>
                  {moreItems.map(({ id, label }) => (
                    <button key={id} onClick={() => { setPage(id); setMoreOpen(false); }} style={{ display: 'block', width: '100%', background: page===id ? 'rgba(201,168,76,0.08)' : 'none', border: 'none', cursor: 'pointer', fontFamily: "'Oswald',sans-serif", fontSize: 12, letterSpacing: '0.12em', textTransform: 'uppercase', color: page===id ? '#C9A84C' : '#D6CBBA', padding: '13px 20px', textAlign: 'left', borderLeft: page===id ? '2px solid #C9A84C' : '2px solid transparent', transition: 'all 0.1s' }}>
                      {label}
                    </button>
                  ))}
                </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            {/* Lang toggle */}
            <button onClick={() => setLang(lang === 'EN' ? 'ZH' : 'EN')} style={{ background: 'rgba(201,168,76,0.1)', border: '1px solid rgba(201,168,76,0.3)', borderRadius: 4, padding: '4px 10px', cursor: 'pointer', fontFamily: "'Oswald',sans-serif", fontSize: 11, letterSpacing: '0.08em', color: '#C9A84C' }}>
              {lang === 'EN' ? '中文' : 'EN'}
            </button>
            {/* Login / Account */}
            {user ? (
              <button onClick={() => setPage('account')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'Oswald',sans-serif", fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#D6CBBA' }}>{t.nav_account}</button>
            ) : (
              <button onClick={() => setPage('login')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'Oswald',sans-serif", fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#D6CBBA' }}>{t.nav_login}</button>
            )}
            {/* Cart */}
            <button onClick={() => setCartOpen(true)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#D6CBBA', display: 'flex', alignItems: 'center', gap: 6 }}>
              <IconCart n={totalItems} />
            </button>
            {/* Mobile hamburger */}
            <button onClick={() => setMobileOpen(!mobileOpen)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#D6CBBA', display: 'none' }} className="mobile-menu-btn">
              <IconMenu />
            </button>
          </div>
        </div>
        {/* Mobile menu */}
        {mobileOpen && (
          <div style={{ background: '#0A2A15', borderTop: '1px solid rgba(201,168,76,0.15)', padding: '12px 0' }}>
            {[...navItems, ...moreItems].map(({ id, label }) => (
              <button key={id} onClick={() => { setPage(id); setMobileOpen(false); }} style={{ display: 'block', width: '100%', background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'Oswald',sans-serif", fontSize: 14, letterSpacing: '0.12em', textTransform: 'uppercase', color: page === id ? '#C9A84C' : '#D6CBBA', padding: '14px 24px', textAlign: 'left', borderLeft: page === id ? '3px solid #C9A84C' : '3px solid transparent' }}>
                {label}
              </button>
            ))}
            <div style={{ borderTop: '1px solid rgba(201,168,76,0.1)', margin: '8px 0', padding: '12px 24px 0', display: 'flex', gap: 12 }}>
              <button onClick={() => { setPage(user ? 'account' : 'login'); setMobileOpen(false); }} style={{ background: 'none', border: '1px solid rgba(201,168,76,0.3)', borderRadius: 4, padding: '8px 16px', cursor: 'pointer', fontFamily: "'Oswald',sans-serif", fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#C9A84C' }}>{user ? t.nav_account : t.nav_login}</button>
              <button onClick={() => setLang(lang === 'EN' ? 'ZH' : 'EN')} style={{ background: 'rgba(201,168,76,0.1)', border: '1px solid rgba(201,168,76,0.3)', borderRadius: 4, padding: '8px 14px', cursor: 'pointer', fontFamily: "'Oswald',sans-serif", fontSize: 11, letterSpacing: '0.08em', color: '#C9A84C' }}>{lang === 'EN' ? '中文' : 'EN'}</button>
            </div>
          </div>
        )}
      </nav>
...
function Footer({ setPage, lang }) {
  const t = window.LANG[lang];
  return (
    <footer style={{ background: '#0A1A0F', borderTop: '1px solid rgba(201,168,76,0.15)', paddingTop: 48 }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px 48px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 40 }}>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
            ...
            <div style={{ fontFamily: "'Playfair Display SC',serif", fontSize: 14, fontWeight: 700, color: '#F5F0E8' }}>Pickle Master</div>
          </div>
          <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, color: '#9E8E78', lineHeight: 1.7, maxWidth: 220 }}>{t.footer_about}</p>
          <div style={{ display: 'flex', gap: 12, marginTop: 16 }}>
            {['IG', 'FB'].map(s => (
              <a key={s} href="#" style={{ width: 32, height: 32, border: '1px solid rgba(201,168,76,0.3)', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Oswald',sans-serif", fontSize: 10, color: '#C9A84C', textDecoration: 'none', letterSpacing: '0.05em' }}>{s}</a>
            ))}
          </div>
        </div>
        {/* Quick links */}
        <div>
          <div style={{ fontFamily: "'Oswald',sans-serif", fontSize: 11, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#C9A84C', marginBottom: 16 }}>{t.footer_links}</div>
          {[['home','nav_home'],['member','nav_member'],['book','nav_book'],['social','nav_social'],['private','nav_private'],['contact','nav_contact'],['coaches','nav_coaches'],['tournament','nav_tournament'],['shop','nav_shop'],['faq','nav_faq']].map(([id, key]) => (
            <button key={id} onClick={() => setPage(id)} style={{ display: 'block', background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", fontSize: 13, color: '#9E8E78', padding: '4px 0', textAlign: 'left' }}>{t[key]}</button>
          ))}
        </div>
        {/* Legal */}
        <div>
          <div style={{ fontFamily: "'Oswald',sans-serif", fontSize: 11, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#C9A84C', marginBottom: 16 }}>{t.footer_legal}</div>
          {[t.footer_privacy, t.footer_terms, t.footer_cancel].map(s => (
            <div key={s} style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, color: '#9E8E78', padding: '4px 0', cursor: 'pointer' }}>{s}</div>
          ))}
        </div>
        {/* Address */}
        <div>
          <div style={{ fontFamily: "'Oswald',sans-serif", fontSize: 11, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#C9A84C', marginBottom: 16 }}>{t.home_address}</div>
          <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, color: '#9E8E78', lineHeight: 1.7 }}>{t.home_address_val}</div>
          <div style={{ fontFamily: "'Oswald',sans-serif", fontSize: 11, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#C9A84C', marginBottom: 8, marginTop: 20 }}>{t.footer_payment}</div>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {['Visa','MC','Amex','ApplePay','UnionPay'].map(p => (
              <span key={p} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 4, padding: '3px 8px', fontFamily: "'DM Sans',sans-serif", fontSize: 10, color: '#9E8E78' }}>{p}</span>
            ))}
          </div>
        </div>
      </div>
      <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '16px 24px', textAlign: 'center', fontFamily: "'DM Sans',sans-serif", fontSize: 12, color: '#9E8E78' }}>
        {t.footer_rights}
      </div>
    </footer>
  );
...
function CartSidebar({ open, onClose, cart, setCart, lang, setPage, user }) {
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 380, background: '#F5F0E8', zIndex: 201, display: 'flex', flexDirection: 'column', boxShadow: '-8px 0 32px rgba(0,0,0,0.2)' }}>
        {/* Header */}
        <div style={{ background: '#0F3D24', padding: '20px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ fontFamily: "'Playfair Display SC',serif", fontSize: 18, fontWeight: 700, color: '#F5F0E8' }}>{checkout ? t.checkout_title : t.cart_title}</div>
            ...
            {cart.length === 0 ? (
              ...
              <>
                <div style={{ flex: 1, overflowY: 'auto', padding: 24, display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  {cart.map((item, i) => (
                    <div key={i} style={{ background: '#fff', borderRadius: 10, padding: '14px', boxShadow: '0 2px 8px rgba(15,61,36,0.08)', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                      <div style={{ flex: 1 }}>
                ...
                <div style={{ padding: 24, borderTop: '1px solid #D6CBBA' }}>
